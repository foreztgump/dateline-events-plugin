# sandbox-profiler Specification

## Purpose
The `@dateline/sandbox-profiler` tool measures each sandboxed plugin handler's CPU time and subrequest count against the Cloudflare sandbox budgets (50ms CPU / 10 subrequests), gating CI via `pnpm sandbox:profile`. This spec governs how elapsed time is measured so that the production gate uses real wall-clock timing while unit tests remain deterministic and free of CI-runner-load flakiness.
## Requirements
