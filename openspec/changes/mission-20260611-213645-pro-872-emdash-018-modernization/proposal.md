...
- **M2 / WS2+WS3 (PRO-874/875/876/877):** convert core/rsvp/importer to sandboxed format (`emdash-plugin.jsonc` + single `src/plugin.ts` `satisfies SandboxedPlugin`); RSVP capacity → storage-backed D1 counters + oversell test; cron/lifecycle registration per verified 0.18 shapes; importer implements `ctx.http.fetch` remote feeds under `network:request:unrestricted` or drops the capability.
