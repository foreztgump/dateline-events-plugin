...
in the site. Then `import offlineProbe from "offline-probe"` and pass
it into `emdash({ sandboxed: [offlineProbe] })`.

## Publish

```sh
emdash-plugin login        # if you're not already logged in
emdash-plugin bundle       # produces dist/offline-probe-<version>.tar.gz
