...
- Sandboxed plugins are capped at **50ms CPU + 10 subrequests per invocation**. Heavy work belongs in native plugins or deferred via `ctx.waitUntil`. Gate every sandboxed handler in CI.
